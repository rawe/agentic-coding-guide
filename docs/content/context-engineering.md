...
<!-- Visualization: Vertical numbered layer stack (7 rows)
Each row has a circled number, a bold name, and a description. Rows are color-coded on a gradient from blue (top) through purple to green (bottom).
...
<!-- Visualization: Horizontal token budget meter bar
Title: "Token Budget — Typical Session" | 200k context window
Stacked horizontal bar segments (left to right):
- Conversation history (blue, ~25%)
- File reads (purple, ~20%)
- Tool definitions (indigo, ~10%)
- Instructions/CLAUDE.md (green, ~8%)
- Reasoning headroom (amber, ~12%)
- Free space (transparent, ~25%)
A vertical threshold line at 75% labeled "75% — performance degrades"
Legend below with color dots for each segment. -->
...
<!-- Visualization: Vertical control stack (6 rows), each with a colored left border
Each row has three columns: category label (mono, uppercase), feature name + description, and an example (mono, right-aligned).
Rows from top to bottom:
1. ALWAYS-ON (blue border) — CLAUDE.md: Loaded every session — keep it lean, treat it like code | ~200–500 tokens
2. SCOPED (lighter blue border) — Rules files: Modular project instructions; can be scoped to specific file paths | .claude/rules/
3. ON-DEMAND (indigo border) — Skills: Lazy-loaded domain knowledge, triggered by description match | .claude/skills/
4. RUNTIME (purple border) — Tools & MCP servers: Agent pulls what it needs, when it needs it | tool definitions + results
5. ISOLATED (magenta border) — Subagents: Separate context windows for exploration and review | Task tool → independent window
6. ZERO-COST (green border) — Hooks: Run scripts deterministically at lifecycle events — minimal context cost | .claude/settings.json -->
...
**CLAUDE.md** — Only include what Claude can't infer from code. For each line, ask: would removing this cause mistakes? If not, cut it. Prune regularly.
...
**Subagents** — Use subagents for codebase exploration to keep your main context clean. The exploration results stay in the subagent's window — only the summary returns.
...
**Session hygiene** — `/clear` resets the window. A clean session with a better prompt beats a cluttered session with corrections stacked on corrections.
...
**Delegation** — Tell Claude to read files and run commands rather than pasting everything in. Claude fetches selectively; you tend to dump everything.
...
Left card (red/warning accent, "Poor Context Management") lists anti-patterns.
Right card (green accent, "Good Context Management") lists best practices.
Each card has a colored left border and a list of bullet points. -->
